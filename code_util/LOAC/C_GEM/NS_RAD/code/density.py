...
https://github.com/bjornaa/seawater/tree/master
similar to the MATLAB toolboxes SEAWATER from CSIRO and parts of OCEANS from Woods Hole Institute.
Most of the formulas used are taken from Unesco's joint panel on oceanographic tables and standards, UNESCO 1981 and
UNESCO 1983.
...
# The three routines below (_dens0, _seck, dens) are the UNESCO-1983 density
# stack. They are pure scalar arithmetic with no Python objects, and profiling put
# them at ~18% of runtime once the hydrodynamic kernels were compiled -- dens() is
# evaluated twice per grid point per timestep via fun_module.p_bar. They are
# therefore @njit-compiled. Results are bit-identical: same expressions, same
# order, and fastmath is deliberately NOT enabled (it would permit reassociation).
#
# The defaults are 0.0 rather than 0 so numba compiles one float specialisation
# instead of separate int/float ones. 0.1*0 and 0.1*0.0 are both 0.0, so this
# changes nothing numerically.
#
# The remaining routines (svan, sigma, drhodt, alpha, drhods, beta) are unused by
# C-GEM and are left as plain Python; they can still call the compiled dens().
