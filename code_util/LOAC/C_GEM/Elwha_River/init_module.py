...
def init():
    ...
    # Debug: Print min/max width and depth after initialization
    #print(f"Minimum width (B): {min(B):.4f} m, Maximum width (B): {max(B):.4f} m")
    #print(f"Minimum depth (DEPTH): {min(DEPTH):.4f} m, Maximum depth (DEPTH): {max(DEPTH):.4f} m")
    #print("DEPTH values at end of init():", DEPTH)
    #print("DEPTH id in init_module:", id(DEPTH), "first 5 values:", DEPTH[:5])
