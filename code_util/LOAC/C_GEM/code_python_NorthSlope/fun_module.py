...
import math
from config import PI, AMPL, pfun, WARMUP, distance, M, pH_ite, G, mass_mol_B, kISS, wMAX
...
def piston_velocity(t, Uw_sal, Uw_tid, water_temp):
    """Calculate piston velocity for O2 exchange across air-water interface."""
    for i in range(M + 1):
        kflow[i] = math.sqrt(abs(U[i]) * D_O2(t, water_temp) / DEPTH[i])
        if i <= distance:
            kwind[i] = (1.0 / 3.6e5) * 0.31 * (Uw_sal**2) * (Sc(t, i, water_temp) / 660)**-0.5
        else:
            kwind[i] = (1.0 / 3.6e5) * 0.31 * (Uw_tid**2) * (Sc(t, i, water_temp) / 660)**-0.5
