...
maskDir = '/nobackup/dcarrol2/LOAC/mat/river_temperature/mask/';

%%

load([dataDir 'ECCO_V4r5_coastMask_orig.mat']);

binDir = '/nobackup/hzhang1/forcing/jra55_do/river_runoff/';
b2 = binDir;

saveDir = '/nobackupp27/rsavelli/LOAC/ECCO_V4r5/';
writeDir = '/nobackupp27/rsavelli/LOAC/ECCO_V4r5/freshwater_runoff/';
...
if computeGrid
    ...
    cellArea = readbin(['/nobackup/hzhang1/forcing/jra55_do/cellarea.bin'], [nLon, nLat], 1,'real*4',0)';
    ...
    for i = 1:length(runoffLon)
        ...
        jra55.ECCO_V4r5Index(i) = ix;
        jra55.ECCO_V4r5Area(i) = RAC(ix);
        
        jra55.ECCO_V4r5Weight(i) = RAC(ix) ./ cellArea(ir(i));
    ...
    save([saveDir 'jra55_do_ECCO_V4r5_grid_orig.mat']);
    
else
    
    load([saveDir 'jra55_do_ECCO_V4r5_grid_orig.mat']);
...
for i = 33%1:length(years)
    ...
    for j = 1:numDays
        ...
        uniqueIndex = unique(jra55.ECCO_V4r5Index);
        
        for k = 1:length(uniqueIndex)
            
            ix = find(jra55.ECCO_V4r5Index == uniqueIndex(k));
            
            jra55.sumRunoff(k) = sum(jra55.runoff(ix),'omitnan');
            jra55.sumECCO_V4r5Weight(k) = mean(jra55.ECCO_V4r5Weight(ix),'omitnan');
            
            clear ix
            
        end
        
        ECCO_V4r5Runoff = (hFacC .* 0);
        
        ECCO_V4r5Runoff(uniqueIndex) = (jra55.sumRunoff ./ jra55.sumECCO_V4r5Weight);
    
    	%spread amazon runoff 
    	%ix = 44;
    	%iy = 3413;

    	%runoff1 = ECCOV4r5Runoff(ix,iy);
    	...
    	%ECCO_V4r5Runoff(ix,iy) = x1(ir);
    	%ECCO_V4r5Runoff(ix,iy+1) = x1(ir);
    	%ECCO_V4r5Runoff(ix-1,iy) = x1(ir);
	%ECCO_V4r5Runoff(ix-1,iy+1) = x1(ir);
    
        writebin([writeDir fileName(1:end-5) '_ECCO_V4r5_' fileName(end-3:end)],ECCO_V4r5Runoff,1,'real*4',j-1);
       
        disp(num2str(j));
        
        clear ECCO_V4r5Runoff
