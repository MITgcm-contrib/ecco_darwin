...
C
C   BUG FOUND AND FIXED: this was #undef (Steph's original value --
C   correct for HER setup, since her iron/wind files were already on her own
C   model's native 1-degree grid and never needed interpolation). But
C   darwin_exf_load.F's iron_lon0/pCO2_lon0/cldcov_lon0/etc. runtime-
C   interpolation blocks are ALL wrapped in "#ifdef USE_EXF_INTERPOLATION"
C   (confirmed by reading the real source) -- with this undefined, every one
C   of those blocks compiles out entirely, and darwin_exf_load.F/oasim would
C   instead try to read iron (native 144x96), pCO2 (native 2x256), and all
C   11 OASIM atmospheric fields (native 360x180) as if they already matched
C   the model's LLC90 grid directly. All the iron_lon0/pCO2_lon0/data.oasim
C   grid-parameter wiring done this session would have been silently
C   ignored. ecco_darwin's own v06/1deg/code_v4r5_v2/EXF_OPTIONS.h has this
C   #define -- confirmed by diffing against it directly.
