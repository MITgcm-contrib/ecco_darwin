...
for timeStep = 1:numFiles
    ...
    DICTFW = convert2gcmfaces(rdmds([diagDir filename1],ttAverage,'rec',6)) .* mmol_to_mol; %mol m^-3 s^-1
    ...
    forcDIC = mygrid.mskC .* forcDIC .* sStar(:,:,:,nt);
    virtualFluxDIC = mygrid.mskC .* virtualFluxDIC .* sStar(:,:,:,nt);
    
    %biology, mol m^-3 s^-1
    bioDIC = mygrid.mskC .* BIO_DIC .* sStar(:,:,:,nt);
