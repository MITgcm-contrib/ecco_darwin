...
for timeStep = 1:numFiles
    ...
    fluxCO2 = convert2gcmfaces(rdmds([diagDir filename1],ttAverage,'rec',6)) .* mmol_to_mol; %mol m^-3 s^-1
    DICTFW = convert2gcmfaces(rdmds([diagDir filename1],ttAverage,'rec',7)) .* mmol_to_mol; %mol m^-3 s^-1
    ...
    sStarSnap = (mygrid.Depth + ETAN_SNAP(:,:,2)) ./ mygrid.Depth; 
    ...
    %DIC tendency due to dilution, mol m^-3 s^-1
    ...
    BIO_DIC = BIO_DIC - forcDIC; %remove air-sea CO2 flux from gDAR, so it is just biology
    
    forcDIC = mygrid.mskC .* forcDIC .* sStarSnap;
    virtualFluxDIC = mygrid.mskC .* virtualFluxDIC .* sStarSnap;
    
    %biology, mol m^-3 s^-1
    bioDIC = mygrid.mskC .* BIO_DIC .* sStarSnap;
    
    tendDICzs = tendDIC;
    adv_hConvDICzs = adv_hConvDIC;
    adv_vConvDICzs = adv_vConvDIC;
    dif_vConvDICzs = dif_vConvDIC;
    dif_hConvDICzs = dif_hConvDIC;
    forcDICzs = forcDIC;
    bioDICzs =  bioDIC;
    virtualFluxDICzs = 0.*virtualFluxDIC; 
    ...
    intTendDIC = nansum(tendDIC(:,:,1:intLevel) .* convert2gcmfaces(dzMat(:,:,1:intLevel)) .* convert2gcmfaces(mygrid.hFacC(:,:,1:intLevel)),3);
    ...
    intForcDIC = nansum(forcDIC(:,:,1:intLevel) .* convert2gcmfaces(dzMat(:,:,1:intLevel)).* convert2gcmfaces(mygrid.hFacC(:,:,1:intLevel)),3);
    ...
    intBioDIC = nansum(bioDIC(:,:,1:intLevel) .* convert2gcmfaces(dzMat(:,:,1:intLevel)) .* convert2gcmfaces(mygrid.hFacC(:,:,1:intLevel)),3);
    
    intResidual = intTendDIC - (intHAdvDIC + intVAdvDIC ...
    ...
    if plotDICBudget
        ...
        pcolor(mygrid.XC.f1,mygrid.YC.f1,intResidual);
    ...
    sumTend = nansum(nansum(intTendDIC));
    sumResidual = nansum(nansum(intResidual));
