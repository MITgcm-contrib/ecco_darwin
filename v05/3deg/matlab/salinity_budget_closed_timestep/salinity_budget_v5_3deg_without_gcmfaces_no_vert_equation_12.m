...
gridDir = '../../../darwin3/run_equation_12/';
modelDir = '../../../darwin3/run_equation_12/';
...
for timeStep = 6:6 %1:numFiles
    ...
    UVELSLT = rdmds([diagDir filename2],ttAverage,'rec',1);
    VVELSLT = rdmds([diagDir filename2],ttAverage,'rec',2);
    WVELSLT = rdmds([diagDir filename2],ttAverage,'rec',3);
    ...
    VVELSLT_SNAP = nan .* ones(nx,ny,numLevels,2);
    WVELSLT_SNAP = nan .* ones(nx,ny,numLevels,2);
    ...
    if timeStep == 1
        ...
        UVELSLT_SNAP(:,:,:,2) = rdmds([diagDir filename7],tt(1),'rec',1);
        VVELSLT_SNAP(:,:,:,2) = rdmds([diagDir filename7],tt(1),'rec',2);
        WVELSLT_SNAP(:,:,:,2) = rdmds([diagDir filename7],tt(1),'rec',3);
    ...
    elseif timeStep == numFiles %no final snapshot
        ...
        UVELSLT_SNAP(:,:,:,1) = rdmds([diagDir filename7],tt(timeStep-1),'rec',1);
        VVELSLT_SNAP(:,:,:,1) = rdmds([diagDir filename7],tt(timeStep-1),'rec',2);
        WVELSLT_SNAP(:,:,:,1) = rdmds([diagDir filename7],tt(timeStep-1),'rec',3);
        ...
        UVELSLT_SNAP = rdmds([diagDir filename7],ttSnap,'rec',1);
        VVELSLT_SNAP = rdmds([diagDir filename7],ttSnap,'rec',2);
        WVELSLT_SNAP = rdmds([diagDir filename7],ttSnap,'rec',3);
    ...
    for k = 1:numLevels
        ...
        adv_vConvV(:,:,k) = squeeze((WVELSLT(:,:,kp1) .* double(k~=numLevels)) - (WVELSLT(:,:,k) .* double(k~=1))) ...
            ./ (dzMat(:,:,k));   
   ...
   %%
    ...
    %advection
    adv_hConvSal = (-adv_hConvV + adv_hConvS);
    adv_vConvSal = (-adv_vConvV + adv_vConvS);
    
    %diffusion
    dif_hConvSal = dif_hConvS;
    dif_vConvSal = dif_vConvS;
    ...
    if plotSalinityBudget
        ...
        test = -WVELSLT_SNAP(:,:,intLevel,2); 
        
        budget.test = test;
        
        save([saveDir 'budget_equation_12.mat'],'budget');
