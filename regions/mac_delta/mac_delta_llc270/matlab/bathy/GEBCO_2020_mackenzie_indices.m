...
dataDir1 = '/Users/carrolld/Documents/research/mackenzie/mat/corners/'
...
saveDir = '/Users/carrolld/Documents/research/mackenzie/mat/bathy/';

%%

load([dataDir1 'cell_corners.mat']);
...
save([saveDir 'GEBCO_mackenzie_indices_' suffix '.mat'],'bathy','-v7.3');

depth = bathy.medianDepth;
depth(isnan(depth)) = 0;

writebin([saveDir  'GEBCO_mackenzie_bathy_dustin_median.bin'],depth,1,'real*4');
...
XG = readbin([gridDir 'XG.data'],[nx ny]);
YG = readbin([gridDir 'YG.data'],[nx ny]);
Depth = readbin([gridDir 'Depth.data'],[nx ny]);
Depth(Depth == 0) = nan;
...
load(['GEBCO_mackenzie_indices_wet.mat']);

hFig1 = figure(1);
set(hFig1,'units','normalized','outerposition',[0 0 1 1]);
set(gcf,'color',[1 1 1]);

colors = cbrewer('div','Spectral',500);
bgColor = [0.5 0.5 0.5];
...
pcolorcen(XG,YG,bathy.medianDepth);
...
depth1 = readbin([gridDir 'Depth.data'],[nx ny]); %raw bathy
depth2 = bathy.medianDepth; %new

foo = depth1 .* 0;

foo(depth1 == 0 & depth2 == 0) = 0;
foo(depth2 > 0 & depth2 > 0) = 3;

foo(depth1 > 0 & depth2 == 0) = 1;
foo(depth2 > 0 & depth1 == 0) = 2;
