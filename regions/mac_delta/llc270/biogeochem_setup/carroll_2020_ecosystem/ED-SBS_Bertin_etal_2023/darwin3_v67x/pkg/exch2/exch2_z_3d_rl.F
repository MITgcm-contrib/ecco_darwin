...
C     | o Handle exchanges for _RL three-dim zeta-point array.
C     *==========================================================*
...
C     myThid :: My Thread Id. number
      INTEGER myNz
      _RL phi(1-OLx:sNx+OLx,1-OLy:sNy+OLy,myNz,nSx,nSy)
      INTEGER myThid
...
C     msgBuf         :: Informational/error message buffer
      ...
      _RL phiNW(local_maxDim,nSx,nSy)
      _RL phiSE(local_maxDim,nSx,nSy)
...
CEOP
      ...
      IF (useCubedSphereExchange) THEN
       IF ( myNz.GT.local_maxDim ) THEN
         WRITE(msgBuf,'(2A,2(I4,A))') 'EXCH_Z_3D_RL :',
     &     ' 3rd dimension=', myNz,
     &     ' exceeds local_maxDim (=', local_maxDim, ' )'
         CALL PRINT_ERROR( msgBuf , myThid )
         WRITE(msgBuf,'(2A)') 'EXCH_Z_3D_RL :',
     &     ' Increase "local_maxDim" and recompile'
         CALL PRINT_ERROR( msgBuf , myThid )
         STOP 'ABNORMAL END: S/R EXCH_Z_3D_RL'
       ENDIF
...
C-     save 2 corners value (in case we find 1 "missing corner")
      ...
      ENDIF
       ...
       CALL EXCH2_RL1_CUBE( phi, .FALSE., 'T ',
...
C-       North-East corner
           IF ( exch2_isNedge(myTile) .EQ. 1 ) THEN
            DO k=1,myNz
             ...
             DO j=sNy+2,sNy+OLy
              DO i=sNx+2,sNx+OLx
               phi(i,j,k,bi,bj)=e2FillValue_RL
              ENDDO
             ENDDO
#endif
            ENDDO
     ...
     &         exch2_isNedge(myTile) .EQ. 1 ) THEN
           DO k=1,myNz
             ...
             DO j=sNy+2,sNy+OLy
              ...
              ENDDO
             ENDDO
             phi(1-OLx,sNy+1,k,bi,bj)=e2FillValue_RL
#endif
           ENDDO
          ENDIF
     ...
     &         exch2_isSedge(myTile) .EQ. 1 ) THEN
            DO k=1,myNz
             ...
             DO j=1-OLy,0
              ...
              ENDDO
             ENDDO
             phi(sNx+1,1-OLy,k,bi,bj)=e2FillValue_RL
#endif
            ENDDO
          ENDIF
