...
CBOP 0
C !ROUTINE: EXCH2_AD_GET_RL1

C !INTERFACE:
      SUBROUTINE EXCH2_AD_GET_RL1(
...
C     sizeNb        :: Second dimension of e2Bufr1_RL
C     sizeBi        :: Third  dimension of e2Bufr1_RL
C     sizeBj        :: Fourth dimension of e2Bufr1_RL
...
#endif /* ALLOW_USE_MPI */
      ELSE
       STOP 'EXCH2_AD_GET_RL1:: commSetting VALUE IS INVALID'
      ENDIF
...
C     Read from e2Bufr1_RL(iBufr,sNb,sBi,sBj,sLv)
         ...
         iLoc = iBufr
#endif
         e2Bufr1_RL(iLoc,sNb,sBi,sBj,sLv) = array(itl,jtl,ktl)
         array(itl,jtl,ktl) =  0.
        ENDDO
       ENDDO
      ENDDO
      IF ( iBufr .GT. e2BufrRecSize ) THEN
        WRITE(msgBuf,'(2A,I9,A,I9)') 'EXCH2_AD_GET_RL1:',
     &   ' iBufr =', iBufr, ' exceeds E2BUFR size=', e2BufrRecSize
        CALL PRINT_ERROR ( msgBuf, myThid )
        STOP 'ABNORMAL END: S/R EXCH2_AD_GET_RL1 (iBufr over limit)'
      ENDIF
