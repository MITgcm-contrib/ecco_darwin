...
CBOP 0
C !ROUTINE: EXCH2_GET_RS2

C !INTERFACE:
      SUBROUTINE EXCH2_GET_RS2 (
...
#endif /* ALLOW_USE_MPI */
      ELSE
       STOP 'EXCH2_GET_RS2:: commSetting VALUE IS INVALID'
      ENDIF
...
C     Read from e2Bufr1_RS(iBufr,sNb,sBi,sBj,sLv)
         iBufr1 = iBufr1+1
         array1(itl,jtl,ktl) = e2Bufr1_RS(iBufr1,sNb,sBi,sBj,sLv)
        ENDDO
       ENDDO
      ENDDO
...
C     Read from e2Bufr2_RS(iBufr,sNb,sBi,sBj,sLv)
         iBufr2 = iBufr2+1
         array2(itl,jtl,ktl) = e2Bufr2_RS(iBufr2,sNb,sBi,sBj,sLv)
        ENDDO
       ENDDO
      ENDDO
