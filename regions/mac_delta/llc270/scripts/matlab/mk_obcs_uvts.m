...
%{
%TS='from270/';
TS='/nobackup/dcarrol2/v05_latest/darwin3/run/diags/budget/';
TS2='/nobackup/dcarrol2/v05_latest/darwin3/run/diags/monthly/';
tmp=zeros([nx ny nz]);
fla=zeros([nx ny]);

flds={'Theta','Salt','U','V'};
flds2={'t',   's',   'u','v'};
kk=[1 1 1 2]; %T/S/U/V record
for f=1:length(flds)
fld=flds{f};
fld2=flds2{f};
if f==1 %T
%	fnm=dir([TS 'average_theta_3d*data']); %corrupt?
	fnm=dir([TS2 'THETA*data']); %corrupt?
elseif f==2 %S
	fnm=dir([TS 'average_salt_3d*data']);
else	     %U/V
	fnm=dir([TS 'average_velmass_3d*data']);
end
        fnm=fnm(2:end); %b/c 1st is 0000000021.data
